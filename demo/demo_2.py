...
def prefetch_batch_input_shape(model: nn.Module, ori_wh: Tuple[int,
    ...
    data_sample = model.data_preprocessor(data, False)
    batch_input_shape = data_sample['data_samples'][0].batch_input_shape
...
def main():
    ...
    if args.out:
        ...
        video_writer = cv2.VideoWriter(
            args.out, fourcc, video_origin.fps,
    ...
    with torch.no_grad():
        for i, (frame_resize, frame_origin) in enumerate(
            ...
            frame_mask = visualizer._image

            if args.show:
                cv2.namedWindow('video', 0)
                mmcv.imshow(frame_mask, 'video', args.wait_time)
            if args.out:
                # video_writer.write(frame_mask[..., ::-1])
                # mmcv.imwrite(frame_mask, 'work_dirs/1.jpg')
