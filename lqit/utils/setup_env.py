...
def register_all_modules(init_default_scope: bool = True) -> None:
    """Register all modules in lqit into the registries.

    Args:
        init_default_scope (bool): Whether initialize the lqit default scope.
            When `init_default_scope=True`, the global default scope will be
            set to `lqit`, and all registries will build modules from lqit's
    ...
    if init_default_scope:
        never_created = DefaultScope.get_current_instance() is None \
                        or not DefaultScope.check_instance_created('lqit')
        if never_created:
            DefaultScope.get_instance('lqit', scope_name='lqit')
            return
        current_scope = DefaultScope.get_current_instance()
        if current_scope.scope_name != 'lqit':
            warnings.warn('The current default scope '
                          f'"{current_scope.scope_name}" is not "lqit", '
                          '`register_all_modules` will force the current'
                          'default scope to be "lqit". If this is not '
                          'expected, please set `init_default_scope=False`.')
            # avoid name conflict
            new_instance_name = f'lqit-{datetime.datetime.now()}'
            DefaultScope.get_instance(new_instance_name, scope_name='lqit')
