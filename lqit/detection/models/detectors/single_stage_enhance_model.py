...
class SingleStageWithEnhanceModel(SingleStageDetector):
    """Base class for one-stage detectors with enhance model.
    ...
    def with_enhance_model(self) -> bool:
        ...
        return hasattr(self, 'enhance_model') and self.enhance_model is not None
    ...
    def loss(self, batch_inputs: Tensor,
        ...
        if self.with_enhance_model:
            enhance_loss = self.enhance_model.loss(batch_inputs, batch_data_samples)
            # avoid loss override
    ...
    def predict(self,
        ...
        if self.vis_enhance and self.with_enhance_model:
            enhance_list = self.enhance_model.predict(
                x, batch_data_samples, rescale=rescale)
