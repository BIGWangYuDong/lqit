...
from .errors import (BackgroundError, BoxError, ClassError, DuplicateError,
                     ...
                     OtherError, Qualifier)
from .functions import mean, nonepack
from .plotting import Plotter, print_table


class TIDEExample:
    """Computes all the data needed to evaluate a set of predictions and gt for
    a single image."""
    ...
    def _run(self):
        ...
        # Update internally so TIDERun can update itself
        # if :max_dets takes effect
        self.preds = preds
        ...
        # Ignore regions annotations allow us to ignore
        # predictions that fall within
        if len(ignore) > 0:
            # Because ignore regions have extra parameters,
            # it's more efficient to use a for loop here
            for ignore_region in ignore:
                if (ignore_region['mask'] is None
                        and ignore_region['bbox'] is None):
                    # The region should span the whole image
                    ignore_iou = [1] * len(preds)
                else:
                    if ignore_region[det_type] is None:
                        # There is no det_type annotation for this
                        # specific region so skip it
                        continue
                    # Otherwise, compute the crowd IoU between the
                    # detections and this region
                ...
                for pred_idx, pred_elem in enumerate(preds):
                    if (not pred_elem['used']
                            ...
                            and (ignore_region['class'] == pred_elem['class']
...
class TIDERun:
    ...
    # Temporary variables stored in ground truth
    # that we need to clear after a run
    _temp_vars = [
    ...
    def _run(self):
        ...
        for image in self.gt.images:
            ...
            # These classes are ignored for the whole image and
            # not in the ground truth, so we can safely just remove
            # these detections from the predictions at the start.
            # However, since ignored detections are still used for
            # error calculations, we have to keep them.
        ...
        for error in self.errors:
            error.original = nonepack(error.unfix())
            error.fixed = nonepack(error.fix())
    ...
    def _eval_image(self, preds: list, gt: list):
        ...
        ex = TIDEExample(preds, gt, self.pos_thresh, self.mode, self.max_dets,
                         self.run_errors)
        # In case the number of predictions was restricted to the max
        preds = ex.preds
        ...
        for pred_idx, pred in enumerate(preds):
            ...
            if self.run_errors and \
                    (pred['used'] is False or pred['used'] is None):
                # Note this is ex.gt because it doesn't include
                # ignore annotations
                if len(ex.gt) == 0:
                ...
                if self.bg_thresh <= ex.gt_cls_iou[pred_idx,
                                                   idx] <= self.pos_thresh:
                    # This detection would have been positive
                    # if it had higher IoU with this GT
                ...
                if ex.gt_noncls_iou[pred_idx, idx] >= self.pos_thresh:
                    # This detection would have been a positive
                    # if it was the correct class
                ...
                if ex.gt_used_cls[pred_idx, idx] >= self.pos_thresh:
                    # The detection would have been marked positive
                    # but the GT was already in use
        ...
        for truth in gt:
            # If the GT wasn't used in matching, meaning it's
            # some kind of false negative
    ...
    def fix_errors(self,
        ...
        """Returns a ClassedAPDataObject where all errors given the condition
        returns True are fixed."""
        if ap_data is None:
        ...
        # Add the correct amount of GT positives,
        # and also subtract if necessary
        for k, v in gt_pos.items():
            # In case you want to fix all false negatives without
            # affecting precision
    ...
    def fix_main_errors(self,
        ...
        for error in error_types:
            ...
            # If an error is negative that means it's likely due to
            # binning differences, so just Ignore the negative
            # by setting it to 0.
    ...
    def fix_special_errors(self) -> dict:
        return {
            FalsePositiveError:
    ...
    def apply_qualifier(self, qualifier: Qualifier) -> ClassedAPDataObject:
        """Applies a qualifier lambda to the AP object for this runs and stores
        the result in self.qualifiers."""
...
class TIDE:
    ...
    def __init__(self,
        ...
        assert mode in [TIDE.BOX, TIDE.MASK]
        self.mode = mode
    ...
    def evaluate(self,
                 ...
                 mode: str = None,
                 name: str = None,
                 use_for_errors: bool = True) -> TIDERun:
        pos_thresh = self.pos_thresh if pos_threshold is None else pos_threshold  # noqa
        bg_thresh = self.bg_thresh if background_threshold is None else background_threshold  # noqa
    ...
    def add_qualifiers(self, *quals):
        """Applies any number of Qualifier objects to evaluations that have
        been run up to now.
        ...
        # 			# If this was a threshold run, apply
        # 			# the qualifier for every run
        # 			for t_run in self.run_thresholds[run_name]:
        # 				t_run.apply_qualifier(q)
        # 		else:
    ...
    def summarize(self):
        """Summarizes the mAP values and errors for all runs in this TIDE
        object.
        ...
        for run_name, run in self.runs.items():
            ...
            if run_name in self.run_thresholds:
                ...
                aps = [t_run.ap for t_run in thresh_runs]
                ...
                print(f'{ap_title:s}: {sum(aps) / len(aps):.2f}')

                # Print AP for every threshold on a threshold run
                print_table([['Thresh'] + [
                    str(int(t_run.pos_thresh * 100)) for t_run in thresh_runs
                ], ['  AP  '] + [f'{t_run.ap:6.2f}' for t_run in thresh_runs]],
                            title=ap_title)

                # Print qualifiers for a threshold run
                if len(self.qualifiers) > 0:
                    print()
                    # Can someone ban me from using list comprehension?
                    # this is unreadable
                    qAPs = [
                        mean([
                            t_run.qualifiers[q] for t_run in thresh_runs
                            if q in t_run.qualifiers
                        ]) for q in self.qualifiers
                    ]

                    print_table([['Name'] + list(self.qualifiers.keys()),
                                 [' AP '] + [f'{qAP:6.2f}' for qAP in qAPs]],
                                title=f'Qualifiers {ap_title}')
            ...
            print_table(
                [['Type'] + [err.short_name for err in TIDE._error_types],
                 [' dAP'] + [
                     f'{main_errors[run_name][err.short_name]:6.2f}'
            ...
            print_table(
                [['Type'] +
                 [err.short_name
                  for err in TIDE._special_error_types], [' dAP'] + [
                      f'{special_errors[run_name][err.short_name]:6.2f}'
    ...
    def plot(self, out_dir: str = None):
        ...
        Images will be outputted to out_dir, which will be created if it
        doesn't exist.
        """
        ...
        # The max will round up to the nearest unit of this
        dap_granularity = 5
