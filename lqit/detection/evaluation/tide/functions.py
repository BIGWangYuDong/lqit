...
def apply_messy2(x: list, y: list, func):
    return [[func(i, j)
             for i, j in zip(a, b)] if isiterable(a) else func(a, b)
            for a, b in zip(x, y)]
...
class HiddenPrints:
    """Modified from https://stackoverflow.com/questions/8391411/suppress-
    calls-to-print-python."""
...
def polyToBox(poly: list):
    """Converts a polygon in COCO lists of lists format to a bounding box in.
