...
class APDataObject:
    """Stores all the information necessary to calculate the AP for one IoU and
    one class.
    ...
    def apply_qualifier(self, kept_preds: set, kept_gts: set) -> object:
        """Makes a new data object where we remove the ids in the pred and gt
        lists."""
        obj = APDataObject()
        num_gt_removed = 0
        ...
        for pred_id in self.data_points:
            ...
            # If the data point we kept was a true positive,
            # there's a corresponding ground truth.
        ...
        num_gt_removed += (
            len(self.false_negatives) - len(obj.false_negatives))
    ...
    def get_ap(self) -> float:
        ...
        # Compute the precision-recall curve. The x axis is
        # recalls and the y axis precisions.
        for datum in data_points:
            # datum[1] is whether the detection
        ...
        # Smooth the curve by computing
        # [max(precisions[i:]) for i in range(len(precisions))]
        ...
        resolution = 100  # Standard COCO Resolution
        # idx 0 is recall == 0.0 and idx 100 is recall == 1.00
        y_range = [0] * (resolution + 1)
        x_range = np.array([x / resolution for x in range(resolution + 1)])
        recalls = np.array(recalls)

        # I realize this is weird, but all it does is find
        # the nearest precision(x) for a given x in x_range.
        # Basically, if the closest recall we have to 0.01 is 0.009
        # this sets precision(0.01) = precision(0.009).
        # I approximate the integral this way,
        # because that's how COCOEval does it.
...
class APEval:
    """A class that computes the AP of some dataset.
        ...
        Note: TIDE doesn't use this internally. This is here so you can get
    a look at how the AP calculation process works.
    """
    ...
    def _eval_image(self, preds: list, gt: list, type_str: str = 'box'):
        ...
        iou_cache = mask_utils.iou(preds_data, [x[data_type] for x in gt],
                                   [False] * num_gt)

        if num_crowd > 0:
            crowd_iou_cache = mask_utils.iou(preds_data,
            ...
            for iouIdx in range(len(self.iou_thresholds)):
                ...
                for i in indices:
                    ...
                    if max_match_idx >= 0:
                        ...
                        # If the detection matches a crowd,
                        ...
                        # All this crowd code so that we can make sure that
                        # our eval code gives the same result as COCOEval.
                        # There aren't even that many crowd annotations to
    ...
    def compute_mAP(self):
        ...
        for thresh, classes in self.ap_data.items():
            num_classes = len(
                [x for x in classes.values() if not x.is_empty()])
            ap = 0

            if num_classes > 0:
                class_APs = [
                    x.get_ap() for x in classes.values() if not x.is_empty()
                ]
