...
from .functions import polyToBox, toRLE
...
def download_annotations(name: str,
    ...
    # If there isn't a file called _finished, that means
    # we didn't finish downloading last time, so try again
    already_downloaded = (
        os.path.exists(candidate_path) and os.path.exists(finished_file_path))

    if not force_download and already_downloaded:
        return candidate_path
    else:
        print(f'{name} annotations not found. Downloading...')
        ...
        # Make an empty _finished file to mark that we were successful
        open(finished_file_path, 'a').close()
        print(f'Successfully downloaded {name} to "{candidate_path}"')
        return candidate_path


def COCO(path: str = None,
         name: str = None,
         year: int = 2017,
         ann_set: str = 'val',
    ...
    If path is not specified, this will download the COCO annotations for the
    year and ann_set specified. Valid years are 2014, 2017 and valid ann_sets
    are 'val' and 'train'.
    """
    if path is None:
        path = download_annotations(
            f'COCO{year}',
            f'http://images.cocodataset.org/annotations/annotations_trainval{year}.zip',  # noqa
            force_download)

        path = os.path.join(path, 'annotations',
                            f'instances_{ann_set}{year}.json')
...
def COCOResult(path, name=None) -> Data:
...
def LVIS(path: str = None,
         name: str = None,
         version_str: str = 'v1',
         force_download: bool = False) -> Data:
    """Load an LVIS-style dataset. The version string is used for downloading
    the dataset and should be one of the versions of LVIS (e.g., v0.5, v1).

    Note: LVIS evaluation is special, but we can emulate it by
    adding ignore regions. The detector isn't punished for predicted
    class that LVIS annotators haven't guaranteed are in the image
    (i.e., the sum of GT annotated classes in the image and those marked
    explicitly not in the image.) In order to emulate this behavior,
    add ignore region labels for every class not found to be in the image.
    This is not that inefficient because ignore regions are separate out
    during mAP calculation and error processing, so adding a bunch of
    them doesn't hurt.

    The LVIS AP numbers are slightly lower than what
    the LVIS API reports because of these workarounds.
    """
    if path is None:
        path = download_annotations(
            f'LVIS{version_str}',
            f'https://s3-us-west-2.amazonaws.com/dl.fbaipublicfiles.com/LVIS/lvis_{version_str}_val.json.zip',  # noqa
            force_download)

        path = os.path.join(path, f'lvis_{version_str}_val.json')
    ...
    for image in images:
        ...
        data.add_image(image['id'], image['coco_url'])
        # Negative categories are guaranteed by the annotators
        # to not be in the image.
    ...
    for ann in anns:
        ...
        # There's an annotation for this class, so we should
        # consider the class for evaluation.
        classes_in_img[image].add(_class)

    all_classes = set(data.classes.keys())

    # LVIS doesn't penalize the detector for detecting classes
    # that the annotators haven't guaranteed to be in/out of
    # the image. Here we simulate that property by adding ignore
    # regions for all such classes.
    for image in images:
        ...
        # explicitly marked as not exhaustively annotated.
        # We can emulate this by adding ignore regions for every category
        # listed, so add them to the ignored classes.
...
def LVISResult(path: str, name: str = None) -> Data:
    ...
    Note: This is the same as a COCO-style results file.
    """
    return COCOResult(path, name)


def Pascal(path: str = None,
           name: str = None,
           year: int = 2007,
           ann_set: str = 'val',
           force_download: bool = False) -> Data:
    ...
    if path is None:
        path = download_annotations(
            'Pascal',
            'https://s3.amazonaws.com/images.cocodataset.org/external/external_PASCAL_VOC.zip',  # noqa
            force_download)

        path = os.path.join(path, 'PASCAL_VOC', f'pascal_{ann_set}{year}.json')
...
def Cityscapes(path: str, name: str = None):
    ...
    Note that we can't automatically download Cityscapes because it
    requires registration and an agreement to the ToS.
    ...
    for ann in Path(path).glob('*/*.json'):
        ...
        for obj in objs:
            ...
            # Cityscapes labelers can label objects without defined boundaries
            # as 'group'. In COCO-land this would be a crowd annotation.
