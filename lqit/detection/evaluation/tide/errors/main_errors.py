...
class DuplicateError(Error):
    ...
    description = 'Error caused when a prediction would have been marked ' \
        + "positive if the GT wasn't already in use by another detection."
    short_name = 'Dupe'

    def __init__(self, pred: dict, suppressor: dict) -> None:
        self.pred = pred
...
class BackgroundError(Error):
    ...
    description = 'Error caused when this detection should have' \
        + ' been classified as background (IoU < 0.1).'
    short_name = 'Bkg'
...
class OtherError(Error):
    ...
    description = "This detection didn't fall into any of" \
...
class MissedError(Error):
    ...
    description = 'Represents GT missed by the model.' \
        + "Doesn't include GT corrected elsewhere in the model."
    short_name = 'Miss'

    def __init__(self, gt: dict) -> None:
        self.gt = gt
...
# These are special errors so no inheritance
class FalsePositiveError:

    description = 'Represents the potential AP gained by having' \
        + ' perfect precision (e.g., by scoring all false positives' \
        + ' as `conf=0`) without affecting recall.'
    short_name = 'FalsePos'
...
class FalseNegativeError:
    ...
    description = 'Represents the potentially AP gained by having' \
        + 'perfect recall without affecting precision.'
    short_name = 'FalseNeg'
