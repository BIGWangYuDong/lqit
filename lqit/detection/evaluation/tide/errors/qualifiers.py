...
class Qualifier:
    ...
    test_func should be a callable object (e.g., lambda) that takes in as input
    an annotation object (either a ground truth or prediction) and returns
    whether or not that object qualifies (i.e., a bool).
    """

    def __init__(self, name: str, test_func: object) -> None:
        self.test = test_func
        self.name = name

    # This is horrible, but I like it
    def _make_error_func(self, error_type):
        return (lambda err: isinstance(err, error_type) and (self.test(err.gt) if hasattr(err, 'gt') else self.test(err.pred))) \
            if self.test is not None else (lambda err: isinstance(err, error_type))  # noqa
